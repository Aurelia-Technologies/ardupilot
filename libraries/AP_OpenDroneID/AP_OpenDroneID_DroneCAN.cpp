...
#define ODID_COPY_STR(name)                                           \
    do                                                                \
    {                                                                 \
        for (uint8_t i = 0; i < sizeof(pkt.name) && pkt.name[i]; i++) \
            msg.name.push_back(pkt.name[i]);                          \
    } while (0)
...
static void handle_arm_status(AP_UAVCAN *ap_uavcan, uint8_t node_id, const ArmStatusCb &cb)
{
    ...
    if (msg.status == 3)
    {
        char *cptr = reinterpret_cast<char *>(const_cast<char *>(msg.error.c_str()));
        GlobalConstants *gc = GlobalConstants::get_singleton();
        gc->set_rid_value(cptr);
    }
    else
    {
