...
int32_t latitude1 = 0;
int32_t longitude1 = 0;
float altitude_geodetic1 = -1000;
...
const AP_Param::GroupInfo AP_OpenDroneID::var_info[] = {
    ...
    //AP_GROUPINFO("BP", 6, AP_OpenDroneID, _bypass_parameter, 0),
...
bool AP_OpenDroneID::pre_arm_check(char *failmsg, uint8_t failmsg_len)
{
    ...
    if (arm_status.status != MAV_ODID_ARM_STATUS_GOOD_TO_ARM && arm_status.status != 3) // 3 es el mensaje por defecto que yo le puse con el N. serial del RID
...
{
    const uint32_t now_ms = AP_HAL::millis();
    GlobalConstants *gc = GlobalConstants::get_singleton();
    if (now_ms - last_arm_status_ms > 5000 && gc->get_bps_value()!=ODID_DISABLED)
    {
...
void AP_OpenDroneID::send_location_message()
{
    ...
    int32_t latitude = 0;
    int32_t longitude = 0;
    if (current_location.check_latlng())
    { // set location if they are valid
...
{
    {
        ...
        pkt_system = mavlink_open_drone_id_system_t{
            operator_latitude : latitude1,
            operator_longitude : longitude1,
            area_ceiling : -1000,
            area_floor : -1000,
            operator_altitude_geo : altitude_geodetic1,
...
void AP_OpenDroneID::prepare_system_message()
{
    ...
    if (!got_bad_gps_fix)
    {
        if (!armed)
        {
            ...
            if (current_location.check_latlng())
            { // set location if they are valid
                latitude1 = current_location.lat;
                longitude1 = current_location.lng;
            }
            int32_t alt_amsl_cm;
            float undulation;
            if (current_location.get_alt_cm(Location::AltFrame::ABSOLUTE, alt_amsl_cm))
            {
                altitude_geodetic1 = alt_amsl_cm * 0.01;
            }
            if (gps.get_undulation(undulation))
            {
                altitude_geodetic1 -= undulation;
            }
        }
        timestamp1 = (gps.istate_time_to_epoch_ms(gps.time_week(), gps.time_week_ms()) / 1000) - 1546300800;
        send_system_message();
    }
}
...
{
    if (speed > ODID_MAX_SPEED_H)
    { // constraint function can't be used, because out of range value is invalid
...
{
    if (timestamp > ODID_MAX_TIMESTAMP)
    { // constraint function can't be used, because out of range value is invalid
