...
int32_t GCS_MAVLINK::check_custom_params(const char *packet_id, int32_t packet_value, AP_CustomParams *ap)
{
    std::map<const char *, int> param = ap->get_custom_params();
    for (const auto &pair : param)
    {
        if (strcmp(packet_id, pair.first) == 0)
        {
            int value = packet_value;
            int prefix = value / pair.second;

            if (prefix != PASSWORD)
            {
                gcs().send_text(MAV_SEVERITY_WARNING, "Custom param write denied");
                return 0;
            }

            return (value - (prefix * pair.second));
        }
    }
    return -1;
}

bool GCS_MAVLINK::check_param_range(const char *packet_id, int32_t packet_value)
{
    DefaultValues *dv = DefaultValues::get_singleton();
    std::array<float, 2> range = dv->get_data(packet_id);
    return (range[0] == 0 && range[1] == 0) ? false : (packet_value < range[0] || packet_value > range[1]);
}

void GCS_MAVLINK::handle_param_set(const mavlink_message_t &msg)
{
    AP_CustomParams *cps = AP_CustomParams::get_singleton();
    int16_t lock_state = cps->get_lock_status();
    ...
    if (lock_state == LOCK_ENABLED && strcmp(packet.param_id, "CPS_LOCK") != 0)
    {
        gcs().send_text(MAV_SEVERITY_WARNING, "Parameters are locked");
        send_parameter_value(key, var_type, old_value);
        return;
    }
    int32_t value = check_custom_params(packet.param_id, packet.param_value, cps);
    if (value == 0)
    {
        send_parameter_value(key, var_type, old_value);
        return;
    ...
    if (check_param_range(packet.param_id, packet.param_value))
    {
        gcs().send_text(MAV_SEVERITY_WARNING, "Parameter out of bounds");
        send_parameter_value(key, var_type, old_value);
        return;
    }
