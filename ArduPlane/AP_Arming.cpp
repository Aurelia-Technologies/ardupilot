...
bool AP_Arming_Plane::pre_arm_checks(bool display_failure)
{
    ...
    if (bpv != plane.cps.get_bps())
        gc->set_bps_value(plane.cps.get_bps());
    bool odid = (gc->get_rid_value() == plane.cps.get_rid() || bpv == ODID_DISABLED);
    if (!odid)
    {
        // 42 lenght max
        check_failed(ARMING_CHECK_ODID, display_failure, "Remote ID does not match FC");
    }
    bool ret = (AP_Arming::pre_arm_checks(display_failure) && odid);
