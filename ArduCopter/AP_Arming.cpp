...
{   
    ...
    int bpv = gc->get_bps_value();
    if (bpv != copter.cps.get_bps())
        gc->set_bps_value(copter.cps.get_bps());
    bool odid = (gc->get_rid_value() == copter.cps.get_rid() || bpv == ODID_DISABLED);
    if (!odid)
    {
        // 42 lenght max
        check_failed(ARMING_CHECK_ODID, display_failure, "Remote ID does not match FC");
    }
    const bool passed = (odid && run_pre_arm_checks(display_failure));
...
bool AP_Arming_Copter::run_pre_arm_checks(bool display_failure)
{
    ...
    return parameter_checks(display_failure) & oa_checks(display_failure) & gcs_failsafe_check(display_failure) & winch_checks(display_failure) & rc_throttle_failsafe_checks(display_failure) & alt_checks(display_failure)
...
bool AP_Arming_Copter::rc_calibration_checks(bool display_failure)
{
    ...
    copter.ap.pre_arm_rc_check = rc_checks_copter_sub(display_failure, channels) & AP_Arming::rc_calibration_checks(display_failure);
